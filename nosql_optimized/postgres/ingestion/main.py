...
from database.postgres_writer import insert_measurements_pg_nosql
...
def run_pipeline(max_locations=30, pages_measurements=1):
        ...
        for s in sensors:
            for page in range(1, pages_measurements + 1):
                ...
                for m in measurements:
                    ...
                    docs.append({
                        "timestamp": datetime.fromisoformat(m["period"]["datetimeFrom"]["utc"].replace("Z","+00:00")),
                        "value": float(val),
                        "sensor": {"id": s["id"], "parameter": s.get("parameter", {}).get("name")},
                        "location": {
                            "id": loc["id"],
    ...
    insert_measurements_pg_nosql(docs)
    print(f"Count : {len(docs)} measurements.")
